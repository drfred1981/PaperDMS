...
import fr.smartprod.paperdms.similarity.service.SimilarityClusterQueryService;
...
public class SimilarityClusterResource {
    ...
    private final SimilarityClusterQueryService similarityClusterQueryService;
    ...
    public SimilarityClusterResource(
        ...
        SimilarityClusterQueryService similarityClusterQueryService
    ) {
        this.similarityClusterService = similarityClusterService;
        this.similarityClusterRepository = similarityClusterRepository;
        this.similarityClusterQueryService = similarityClusterQueryService;
    }
    ...
    public ResponseEntity<List<SimilarityClusterDTO>> getAllSimilarityClusters(
        SimilarityClusterCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get SimilarityClusters by criteria: {}", criteria);

        Page<SimilarityClusterDTO> page = similarityClusterQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /similarity-clusters/count} : count all the similarityClusters.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countSimilarityClusters(SimilarityClusterCriteria criteria) {
        LOG.debug("REST request to count SimilarityClusters by criteria: {}", criteria);
        return ResponseEntity.ok().body(similarityClusterQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /similarity-clusters/_search?query=:query} : search for the similarityCluster corresponding
     * to the query.
     *
     * @param query the query of the similarityCluster search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<SimilarityClusterDTO>> searchSimilarityClusters(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of SimilarityClusters for query {}", query);
        try {
            Page<SimilarityClusterDTO> page = similarityClusterService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
