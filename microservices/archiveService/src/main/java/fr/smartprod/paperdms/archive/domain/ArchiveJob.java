...
public class ArchiveJob implements Serializable {
    ...
    @OneToMany(fetch = FetchType.LAZY, mappedBy = "archiveJob")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "archiveJob" }, allowSetters = true)
    private Set<ArchiveDocument> archivesDocuments = new HashSet<>();
    ...
    public Set<ArchiveDocument> getArchivesDocuments() {
        return this.archivesDocuments;
    }

    public void setArchivesDocuments(Set<ArchiveDocument> archiveDocuments) {
        if (this.archivesDocuments != null) {
            this.archivesDocuments.forEach(i -> i.setArchiveJob(null));
        }
        if (archiveDocuments != null) {
            archiveDocuments.forEach(i -> i.setArchiveJob(this));
        }
        this.archivesDocuments = archiveDocuments;
    }

    public ArchiveJob archivesDocuments(Set<ArchiveDocument> archiveDocuments) {
        this.setArchivesDocuments(archiveDocuments);
        return this;
    }

    public ArchiveJob addArchivesDocuments(ArchiveDocument archiveDocument) {
        this.archivesDocuments.add(archiveDocument);
        archiveDocument.setArchiveJob(this);
        return this;
    }

    public ArchiveJob removeArchivesDocuments(ArchiveDocument archiveDocument) {
        this.archivesDocuments.remove(archiveDocument);
        archiveDocument.setArchiveJob(null);
        return this;
    }
