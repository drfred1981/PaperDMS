...
export class NotificationComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && NotificationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && NotificationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: INotification[] | null): INotification[] {
    ...
    if (this.links().prev) {
      const notificationsNew = this.notifications();
      if (data) {
        for (const d of data) {
          if (notificationsNew.some(op => op.id === d.id)) {
            notificationsNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.notificationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
