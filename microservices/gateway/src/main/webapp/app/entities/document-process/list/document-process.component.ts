...
export class DocumentProcessComponent implements OnInit {
  ...
  protected fillComponentAttributesFromResponseBody(data: IDocumentProcess[] | null): IDocumentProcess[] {
    ...
    if (this.links().prev) {
      const documentProcessesNew = this.documentProcesses();
      if (data) {
        for (const d of data) {
          if (documentProcessesNew.some(op => op.id === d.id)) {
            documentProcessesNew.push(d);
          }
        }
      }
      return documentProcessesNew;
    }
    return data ?? [];
  }
