...
import documentTypeResolve from './document-type-routing-resolve.service';

describe('DocumentType routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: DocumentTypeService;
  let resultDocumentType: IDocumentType | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IDocumentType returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        documentTypeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultDocumentType).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IDocumentType>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
