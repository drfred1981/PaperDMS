import { Component, inject } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';

import SharedModule from 'app/shared/shared.module';
import { ITEM_DELETED_EVENT } from 'app/config/navigation.constants';
import { IMetaMetaTagCategory } from '../meta-meta-tag-category.model';
import { MetaMetaTagCategoryService } from '../service/meta-meta-tag-category.service';

@Component({
  templateUrl: './meta-meta-tag-category-delete-dialog.component.html',
  imports: [SharedModule, FormsModule],
})
export class MetaMetaTagCategoryDeleteDialogComponent {
  metaMetaTagCategory?: IMetaMetaTagCategory;

  protected metaMetaTagCategoryService = inject(MetaMetaTagCategoryService);
  protected activeModal = inject(NgbActiveModal);

  cancel(): void {
    this.activeModal.dismiss();
  }

  confirmDelete(id: number): void {
    this.metaMetaTagCategoryService.delete(id).subscribe(() => {
      this.activeModal.close(ITEM_DELETED_EVENT);
    });
  }
}
