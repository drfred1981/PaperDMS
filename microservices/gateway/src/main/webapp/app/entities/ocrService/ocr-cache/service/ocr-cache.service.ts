...
export class OcrCacheService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/ocr-caches/_search', 'ocrservice');
  ...
  update(ocrCache: IOcrCache): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(ocrCache);
    return this.http
      .put<RestOcrCache>(`${this.resourceUrl}/${this.getOcrCacheIdentifier(ocrCache)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(ocrCache: PartialUpdateOcrCache): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(ocrCache);
    return this.http
      .patch<RestOcrCache>(`${this.resourceUrl}/${this.getOcrCacheIdentifier(ocrCache)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestOcrCache>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestOcrCache[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),

      catchError(() => scheduled([new HttpResponse<IOcrCache[]>()], asapScheduler)),
    );
  }
