...
class DocumentAuditResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/document-audits/_search";
    ...
    @Autowired
    private DocumentAuditSearchRepository documentAuditSearchRepository;
    ...
    void cleanup() {
        if (insertedDocumentAudit != null) {
            ...
            documentAuditSearchRepository.delete(insertedDocumentAudit);
    ...
    void createDocumentAudit() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(documentAuditSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(documentAuditSearchRepository.findAll());
    ...
    @Test
    @Transactional
    void getAllDocumentAuditsByDocumentIsEqualToSomething() throws Exception {
        Document document;
        ...
        // Get all the documentAuditList where document equals to documentId
        defaultDocumentAuditShouldBeFound("documentId.equals=" + documentId);

        // Get all the documentAuditList where document equals to (documentId + 1)
        defaultDocumentAuditShouldNotBeFound("documentId.equals=" + (documentId + 1));
    }
    ...
    void putExistingDocumentAudit() throws Exception {
        ...
        documentAuditSearchRepository.save(documentAudit);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(documentAuditSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(documentAuditSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<DocumentAudit> documentAuditSearchList = Streamable.of(documentAuditSearchRepository.findAll()).toList();
                DocumentAudit testDocumentAuditSearch = documentAuditSearchList.get(searchDatabaseSizeAfter - 1);

                assertDocumentAuditAllPropertiesEquals(testDocumentAuditSearch, updatedDocumentAudit);
            });
    }
    ...
    @Test
    @Transactional
    void searchDocumentAudit() throws Exception {
        // Initialize the database
        insertedDocumentAudit = documentAuditRepository.saveAndFlush(documentAudit);
        documentAuditSearchRepository.save(documentAudit);

        // Search the documentAudit
        restDocumentAuditMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + documentAudit.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(documentAudit.getId().intValue())))
            .andExpect(jsonPath("$.[*].documentSha256").value(hasItem(DEFAULT_DOCUMENT_SHA_256)))
