...
import fr.smartprod.paperdms.document.service.DocumentMetadataQueryService;
...
/**
 * REST controller for managing {@link fr.smartprod.paperdms.document.domain.DocumentMetadata}.
 */
@RestController
...
public class DocumentMetadataResource {
    ...
    private final DocumentMetadataQueryService documentMetadataQueryService;

    public DocumentMetadataResource(
        DocumentMetadataService documentMetadataService,
        DocumentMetadataRepository documentMetadataRepository,
        DocumentMetadataQueryService documentMetadataQueryService
    ) {
     ...
     */
    @PostMapping("")
    public ResponseEntity<DocumentMetadataDTO> createDocumentMetadata(@Valid @RequestBody DocumentMetadataDTO documentMetadataDTO)
    ...
    public ResponseEntity<DocumentMetadataDTO> updateDocumentMetadata(
        ...
        @Valid @RequestBody DocumentMetadataDTO documentMetadataDTO
    ) throws URISyntaxException {
    ...
    public ResponseEntity<DocumentMetadataDTO> partialUpdateDocumentMetadata(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody DocumentMetadataDTO documentMetadataDTO
    ) throws URISyntaxException {
    ...
    public ResponseEntity<List<DocumentMetadataDTO>> getAllDocumentMetadata(
        DocumentMetadataCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get DocumentMetadata by criteria: {}", criteria);

        Page<DocumentMetadataDTO> page = documentMetadataQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /document-metadata/count} : count all the documentMetadata.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDocumentMetadata(DocumentMetadataCriteria criteria) {
        LOG.debug("REST request to count DocumentMetadata by criteria: {}", criteria);
        return ResponseEntity.ok().body(documentMetadataQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /document-metadata/_search?query=:query} : search for the documentMetadata corresponding
     * to the query.
     *
     * @param query the query of the documentMetadata search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentMetadataDTO>> searchDocumentMetadata(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of DocumentMetadata for query {}", query);
        try {
            Page<DocumentMetadataDTO> page = documentMetadataService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
