...
import fr.smartprod.paperdms.document.service.DocumentStatisticsQueryService;
...
public class DocumentStatisticsResource {
    ...
    private final DocumentStatisticsQueryService documentStatisticsQueryService;
    ...
    public DocumentStatisticsResource(
        ...
        DocumentStatisticsQueryService documentStatisticsQueryService
    ) {
        this.documentStatisticsService = documentStatisticsService;
        this.documentStatisticsRepository = documentStatisticsRepository;
        this.documentStatisticsQueryService = documentStatisticsQueryService;
    }
    ...
    public ResponseEntity<List<DocumentStatisticsDTO>> getAllDocumentStatistics(
        DocumentStatisticsCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get DocumentStatistics by criteria: {}", criteria);

        Page<DocumentStatisticsDTO> page = documentStatisticsQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /document-statistics/count} : count all the documentStatistics.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDocumentStatistics(DocumentStatisticsCriteria criteria) {
        LOG.debug("REST request to count DocumentStatistics by criteria: {}", criteria);
        return ResponseEntity.ok().body(documentStatisticsQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /document-statistics/_search?query=:query} : search for the documentStatistics corresponding
     * to the query.
     *
     * @param query the query of the documentStatistics search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentStatisticsDTO>> searchDocumentStatistics(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of DocumentStatistics for query {}", query);
        try {
            Page<DocumentStatisticsDTO> page = documentStatisticsService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
