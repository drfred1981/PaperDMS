...
import fr.smartprod.paperdms.document.service.DocumentTypeFieldQueryService;
...
public class DocumentTypeFieldResource {
    ...
    private final DocumentTypeFieldQueryService documentTypeFieldQueryService;
    ...
    public DocumentTypeFieldResource(
        ...
        DocumentTypeFieldQueryService documentTypeFieldQueryService
    ) {
        this.documentTypeFieldService = documentTypeFieldService;
        this.documentTypeFieldRepository = documentTypeFieldRepository;
        this.documentTypeFieldQueryService = documentTypeFieldQueryService;
    }
    ...
    public ResponseEntity<List<DocumentTypeFieldDTO>> getAllDocumentTypeFields(
        DocumentTypeFieldCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get DocumentTypeFields by criteria: {}", criteria);

        Page<DocumentTypeFieldDTO> page = documentTypeFieldQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /document-type-fields/count} : count all the documentTypeFields.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDocumentTypeFields(DocumentTypeFieldCriteria criteria) {
        LOG.debug("REST request to count DocumentTypeFields by criteria: {}", criteria);
        return ResponseEntity.ok().body(documentTypeFieldQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /document-type-fields/_search?query=:query} : search for the documentTypeField corresponding
     * to the query.
     *
     * @param query the query of the documentTypeField search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentTypeFieldDTO>> searchDocumentTypeFields(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of DocumentTypeFields for query {}", query);
        try {
            Page<DocumentTypeFieldDTO> page = documentTypeFieldService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
