...
public class OcrJobResource {
    ...
    /**
     * {@code SEARCH  /ocr-jobs/_search?query=:query} : search for the ocrJob corresponding
     * to the query.
     *
     * @param query the query of the ocrJob search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<OcrJobDTO>> searchOcrJobs(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of OcrJobs for query {}", query);
        try {
            Page<OcrJobDTO> page = ocrJobService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
