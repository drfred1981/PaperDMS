...
class OcrJobResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/ocr-jobs/_search";
    ...
    @Autowired
    private OcrJobSearchRepository ocrJobSearchRepository;
    ...
    void cleanup() {
        if (insertedOcrJob != null) {
            ...
            ocrJobSearchRepository.delete(insertedOcrJob);
            insertedOcrJob = null;
        }
    }
    ...
    void createOcrJob() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(ocrJobSearchRepository.findAll());
        // Create the OcrJob
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(ocrJobSearchRepository.findAll());
    ...
    void putExistingOcrJob() throws Exception {
        ...
        ocrJobSearchRepository.save(ocrJob);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(ocrJobSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(ocrJobSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<OcrJob> ocrJobSearchList = Streamable.of(ocrJobSearchRepository.findAll()).toList();
                OcrJob testOcrJobSearch = ocrJobSearchList.get(searchDatabaseSizeAfter - 1);

                assertOcrJobAllPropertiesEquals(testOcrJobSearch, updatedOcrJob);
            });
    }
    ...
    @Test
    @Transactional
    void searchOcrJob() throws Exception {
        // Initialize the database
        insertedOcrJob = ocrJobRepository.saveAndFlush(ocrJob);
        ocrJobSearchRepository.save(ocrJob);
        ...
        // Search the ocrJob
        restOcrJobMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + ocrJob.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(ocrJob.getId().intValue())))
            .andExpect(jsonPath("$.[*].status").value(hasItem(DEFAULT_STATUS.toString())))
