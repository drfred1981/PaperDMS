...
import fr.smartprod.paperdms.search.service.SearchFacetQueryService;
...
public class SearchFacetResource {
    ...
    private final SearchFacetQueryService searchFacetQueryService;

    public SearchFacetResource(
        SearchFacetService searchFacetService,
        SearchFacetRepository searchFacetRepository,
        SearchFacetQueryService searchFacetQueryService
    ) {
        this.searchFacetService = searchFacetService;
        this.searchFacetRepository = searchFacetRepository;
        this.searchFacetQueryService = searchFacetQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<SearchFacetDTO>> getAllSearchFacets(
        SearchFacetCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get SearchFacets by criteria: {}", criteria);

        Page<SearchFacetDTO> page = searchFacetQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /search-facets/count} : count all the searchFacets.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countSearchFacets(SearchFacetCriteria criteria) {
        LOG.debug("REST request to count SearchFacets by criteria: {}", criteria);
        return ResponseEntity.ok().body(searchFacetQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /search-facets/_search?query=:query} : search for the searchFacet corresponding
     * to the query.
     *
     * @param query the query of the searchFacet search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<SearchFacetDTO>> searchSearchFacets(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of SearchFacets for query {}", query);
        try {
            Page<SearchFacetDTO> page = searchFacetService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
