...
import fr.smartprod.paperdms.search.service.SearchIndexQueryService;
...
public class SearchIndexResource {
    ...
    private final SearchIndexQueryService searchIndexQueryService;

    public SearchIndexResource(
        SearchIndexService searchIndexService,
        SearchIndexRepository searchIndexRepository,
        SearchIndexQueryService searchIndexQueryService
    ) {
        this.searchIndexService = searchIndexService;
        this.searchIndexRepository = searchIndexRepository;
        this.searchIndexQueryService = searchIndexQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<SearchIndexDTO>> getAllSearchIndices(
        SearchIndexCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get SearchIndices by criteria: {}", criteria);

        Page<SearchIndexDTO> page = searchIndexQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /search-indices/count} : count all the searchIndices.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countSearchIndices(SearchIndexCriteria criteria) {
        LOG.debug("REST request to count SearchIndices by criteria: {}", criteria);
        return ResponseEntity.ok().body(searchIndexQueryService.countByCriteria(criteria));
    }
