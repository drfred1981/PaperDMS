...
public class SearchQueryResource {
    ...
    /**
     * {@code SEARCH  /search-queries/_search?query=:query} : search for the searchQuery corresponding
     * to the query.
     *
     * @param query the query of the searchQuery search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<SearchQueryDTO>> searchSearchQueries(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of SearchQueries for query {}", query);
        try {
            Page<SearchQueryDTO> page = searchQueryService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
