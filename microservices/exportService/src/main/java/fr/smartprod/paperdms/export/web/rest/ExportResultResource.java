...
import fr.smartprod.paperdms.export.service.ExportResultQueryService;
...
public class ExportResultResource {
    ...
    private final ExportResultQueryService exportResultQueryService;

    public ExportResultResource(
        ExportResultService exportResultService,
        ExportResultRepository exportResultRepository,
        ExportResultQueryService exportResultQueryService
    ) {
        this.exportResultService = exportResultService;
        this.exportResultRepository = exportResultRepository;
        this.exportResultQueryService = exportResultQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<ExportResultDTO>> getAllExportResults(
        ExportResultCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get ExportResults by criteria: {}", criteria);

        Page<ExportResultDTO> page = exportResultQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /export-results/count} : count all the exportResults.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countExportResults(ExportResultCriteria criteria) {
        LOG.debug("REST request to count ExportResults by criteria: {}", criteria);
        return ResponseEntity.ok().body(exportResultQueryService.countByCriteria(criteria));
    }
