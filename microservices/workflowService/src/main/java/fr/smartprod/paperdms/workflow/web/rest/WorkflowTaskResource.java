...
import fr.smartprod.paperdms.workflow.service.WorkflowTaskQueryService;
...
/**
 * REST controller for managing {@link fr.smartprod.paperdms.workflow.domain.WorkflowTask}.
 */
@RestController
...
public class WorkflowTaskResource {
    ...
    /**
     * {@code SEARCH  /workflow-tasks/_search?query=:query} : search for the workflowTask corresponding
     * to the query.
     *
     * @param query the query of the workflowTask search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<WorkflowTaskDTO>> searchWorkflowTasks(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of WorkflowTasks for query {}", query);
        try {
            Page<WorkflowTaskDTO> page = workflowTaskService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
