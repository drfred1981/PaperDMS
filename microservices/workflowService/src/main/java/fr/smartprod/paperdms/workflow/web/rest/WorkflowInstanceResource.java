...
public class WorkflowInstanceResource {
    ...
    /**
     * {@code SEARCH  /workflow-instances/_search?query=:query} : search for the workflowInstance corresponding
     * to the query.
     *
     * @param query the query of the workflowInstance search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<WorkflowInstanceDTO>> searchWorkflowInstances(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of WorkflowInstances for query {}", query);
        try {
            Page<WorkflowInstanceDTO> page = workflowInstanceService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
