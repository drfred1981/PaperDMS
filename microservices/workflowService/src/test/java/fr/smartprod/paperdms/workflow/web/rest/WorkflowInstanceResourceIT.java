...
class WorkflowInstanceResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/workflow-instances/_search";
    ...
    @Autowired
    private WorkflowInstanceSearchRepository workflowInstanceSearchRepository;
     ...
     */
    public static WorkflowInstance createEntity() {
     ...
     * if they test an entity which requires the current entity.
     */
    public static WorkflowInstance createUpdatedEntity() {
        return new WorkflowInstance()
    ...
    void cleanup() {
        if (insertedWorkflowInstance != null) {
            ...
            workflowInstanceSearchRepository.delete(insertedWorkflowInstance);
    ...
    void createWorkflowInstance() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(workflowInstanceSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(workflowInstanceSearchRepository.findAll());
    ...
    void checkDocumentSha256IsRequired() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(workflowInstanceSearchRepository.findAll());
        // set the field null
        workflowInstance.setDocumentSha256(null);
    ...
    @Test
    @Transactional
    void getAllWorkflowInstancesByDocumentSha256IsEqualToSomething() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);

        // Get all the workflowInstanceList where documentSha256 equals to
    ...
    @Test
    @Transactional
    void getAllWorkflowInstancesByDocumentSha256IsInShouldWork() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);

        // Get all the workflowInstanceList where documentSha256 in
        defaultWorkflowInstanceFiltering(
    ...
    @Test
    @Transactional
    void getAllWorkflowInstancesByDocumentSha256IsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);

        // Get all the workflowInstanceList where documentSha256 is not null
        defaultWorkflowInstanceFiltering("documentSha256.specified=true", "documentSha256.specified=false");
    }

    @Test
    @Transactional
    void getAllWorkflowInstancesByDocumentSha256ContainsSomething() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);

        // Get all the workflowInstanceList where documentSha256 contains
        defaultWorkflowInstanceFiltering(
    ...
    @Test
    @Transactional
    void getAllWorkflowInstancesByDocumentSha256NotContainsSomething() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);

        // Get all the workflowInstanceList where documentSha256 does not contain
        defaultWorkflowInstanceFiltering(
    ...
    void putExistingWorkflowInstance() throws Exception {
        ...
        workflowInstanceSearchRepository.save(workflowInstance);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(workflowInstanceSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(workflowInstanceSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<WorkflowInstance> workflowInstanceSearchList = Streamable.of(workflowInstanceSearchRepository.findAll()).toList();
                WorkflowInstance testWorkflowInstanceSearch = workflowInstanceSearchList.get(searchDatabaseSizeAfter - 1);

                assertWorkflowInstanceAllPropertiesEquals(testWorkflowInstanceSearch, updatedWorkflowInstance);
            });
    }
    ...
    @Test
    @Transactional
    void searchWorkflowInstance() throws Exception {
        // Initialize the database
        insertedWorkflowInstance = workflowInstanceRepository.saveAndFlush(workflowInstance);
        workflowInstanceSearchRepository.save(workflowInstance);

        // Search the workflowInstance
        restWorkflowInstanceMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + workflowInstance.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(workflowInstance.getId().intValue())))
            .andExpect(jsonPath("$.[*].documentSha256").value(hasItem(DEFAULT_DOCUMENT_SHA_256)))
