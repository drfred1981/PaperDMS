...
class DocumentProcessResourceIT {
    ...
    public static DocumentProcess createEntity() {
        return new DocumentProcess().status(DEFAULT_STATUS).documentSha256(DEFAULT_DOCUMENT_SHA_256);
    }
    ...
    public static DocumentProcess createUpdatedEntity() {
        return new DocumentProcess().status(UPDATED_STATUS).documentSha256(UPDATED_DOCUMENT_SHA_256);
    }
    ...
    @Test
    @Transactional
    void getDocumentProcessesByIdFiltering() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        Long id = documentProcess.getId();

        defaultDocumentProcessFiltering("id.equals=" + id, "id.notEquals=" + id);

        defaultDocumentProcessFiltering("id.greaterThanOrEqual=" + id, "id.greaterThan=" + id);

        defaultDocumentProcessFiltering("id.lessThanOrEqual=" + id, "id.lessThan=" + id);
    }

    @Test
    @Transactional
    void getAllDocumentProcessesByStatusIsEqualToSomething() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where status equals to
        defaultDocumentProcessFiltering("status.equals=" + DEFAULT_STATUS, "status.equals=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllDocumentProcessesByStatusIsInShouldWork() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where status in
        defaultDocumentProcessFiltering("status.in=" + DEFAULT_STATUS + "," + UPDATED_STATUS, "status.in=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllDocumentProcessesByStatusIsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where status is not null
        defaultDocumentProcessFiltering("status.specified=true", "status.specified=false");
    }

    @Test
    @Transactional
    void getAllDocumentProcessesByDocumentSha256IsEqualToSomething() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where documentSha256 equals to
        defaultDocumentProcessFiltering(
    ...
    @Test
    @Transactional
    void getAllDocumentProcessesByDocumentSha256IsInShouldWork() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where documentSha256 in
        defaultDocumentProcessFiltering(
    ...
    @Test
    @Transactional
    void getAllDocumentProcessesByDocumentSha256IsNullOrNotNull() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where documentSha256 is not null
        defaultDocumentProcessFiltering("documentSha256.specified=true", "documentSha256.specified=false");
    }

    @Test
    @Transactional
    void getAllDocumentProcessesByDocumentSha256ContainsSomething() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where documentSha256 contains
        defaultDocumentProcessFiltering(
    ...
    @Test
    @Transactional
    void getAllDocumentProcessesByDocumentSha256NotContainsSomething() throws Exception {
        // Initialize the database
        insertedDocumentProcess = documentProcessRepository.saveAndFlush(documentProcess);

        // Get all the documentProcessList where documentSha256 does not contain
        defaultDocumentProcessFiltering(
    ...
    private void defaultDocumentProcessFiltering(String shouldBeFound, String shouldNotBeFound) throws Exception {
        defaultDocumentProcessShouldBeFound(shouldBeFound);
        defaultDocumentProcessShouldNotBeFound(shouldNotBeFound);
    }

    /**
     * Executes the search, and checks that the default entity is returned.
     */
    private void defaultDocumentProcessShouldBeFound(String filter) throws Exception {
        restDocumentProcessMockMvc
     ...
     */
    private void defaultDocumentProcessShouldNotBeFound(String filter) throws Exception {
        restDocumentProcessMockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    void putExistingDocumentProcess() throws Exception {
        ...
        updatedDocumentProcess.status(UPDATED_STATUS).documentSha256(UPDATED_DOCUMENT_SHA_256);
