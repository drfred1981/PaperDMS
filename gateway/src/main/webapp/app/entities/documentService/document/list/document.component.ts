...
export class DocumentComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && DocumentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DocumentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IDocument[] | null): IDocument[] {
    ...
    if (this.links().prev) {
      const documentsNew = this.documents();
      if (data) {
        for (const d of data) {
          if (documentsNew.some(op => op.id === d.id)) {
            documentsNew.push(d);
          }
        }
      }
