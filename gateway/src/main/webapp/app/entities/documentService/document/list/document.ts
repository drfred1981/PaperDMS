...
import { DocumentDeleteDialog } from '../delete/document-delete-dialog';
import { IDocument } from '../document.model';
import { DocumentService, EntityArrayResponseType } from '../service/document.service';
...
export class Document implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && Document.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(document: IDocument): void {
    const modalRef = this.modalService.open(DocumentDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && Document.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.documents.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.documentService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
