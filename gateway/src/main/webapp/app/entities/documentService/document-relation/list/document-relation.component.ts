...
export class DocumentRelationComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['relationType', 'createdBy'];
  ...
  search(query: string): void {
    ...
    if (query && predicate && DocumentRelationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DocumentRelationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IDocumentRelation[] | null): IDocumentRelation[] {
    ...
    if (this.links().prev) {
      const documentRelationsNew = this.documentRelations();
      if (data) {
        for (const d of data) {
          if (documentRelationsNew.some(op => op.id === d.id)) {
            documentRelationsNew.push(d);
          }
        }
      }
      return documentRelationsNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.documentRelationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
