...
import { sampleWithRequiredData } from '../document-type.test-samples';
...
describe('DocumentType Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.documentTypes()[0]).toEqual(expect.objectContaining({ id: 9974 }));
  });

  describe('trackId', () => {
    it('should forward to documentTypeService', () => {
      const entity = { id: 9974 };
      jest.spyOn(service, 'getDocumentTypeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDocumentTypeIdentifier).toHaveBeenCalledWith(entity);
