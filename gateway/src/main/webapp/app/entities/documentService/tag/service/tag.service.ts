...
type RestOf<T extends ITag | NewTag> = Omit<T, 'createdDate'> & {
...
export class TagService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/tags', 'documentservice');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/tags/_search', 'documentservice');
  ...
  update(tag: ITag): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tag);
    return this.http
      .put<RestTag>(`${this.resourceUrl}/${encodeURIComponent(this.getTagIdentifier(tag))}`, copy, { observe: 'response' })
  ...
  partialUpdate(tag: PartialUpdateTag): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tag);
    return this.http
      .patch<RestTag>(`${this.resourceUrl}/${encodeURIComponent(this.getTagIdentifier(tag))}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestTag>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends ITag | NewTag | PartialUpdateTag>(tag: T): RestOf<T> {
    return {
      ...tag,
      createdDate: tag.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTag: RestTag): ITag {
    return {
      ...restTag,
      createdDate: restTag.createdDate ? dayjs(restTag.createdDate) : undefined,
    };
  }
