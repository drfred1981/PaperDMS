...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'readDate' | 'expirationDate' | 'sentDate' | 'createdDate'> & {
...
export class NotificationService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/notifications', 'notificationservice');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/notifications/_search', 'notificationservice');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestNotification[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),

      catchError(() => scheduled([new HttpResponse<INotification[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      readDate: notification.readDate?.toJSON() ?? null,
      expirationDate: notification.expirationDate?.toJSON() ?? null,
      sentDate: notification.sentDate?.toJSON() ?? null,
      createdDate: notification.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      readDate: restNotification.readDate ? dayjs(restNotification.readDate) : undefined,
      expirationDate: restNotification.expirationDate ? dayjs(restNotification.expirationDate) : undefined,
      sentDate: restNotification.sentDate ? dayjs(restNotification.sentDate) : undefined,
      createdDate: restNotification.createdDate ? dayjs(restNotification.createdDate) : undefined,
    };
  }
