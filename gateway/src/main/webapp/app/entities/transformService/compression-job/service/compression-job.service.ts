...
export class CompressionJobService {
  ...
  update(compressionJob: ICompressionJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(compressionJob);
    return this.http
      .put<RestCompressionJob>(`${this.resourceUrl}/${this.getCompressionJobIdentifier(compressionJob)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(compressionJob: PartialUpdateCompressionJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(compressionJob);
    return this.http
      .patch<RestCompressionJob>(`${this.resourceUrl}/${this.getCompressionJobIdentifier(compressionJob)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestCompressionJob>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
