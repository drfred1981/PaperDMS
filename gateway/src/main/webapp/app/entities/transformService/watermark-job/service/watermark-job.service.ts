...
export class WatermarkJobService {
  ...
  update(watermarkJob: IWatermarkJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(watermarkJob);
    return this.http
      .put<RestWatermarkJob>(`${this.resourceUrl}/${encodeURIComponent(this.getWatermarkJobIdentifier(watermarkJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(watermarkJob: PartialUpdateWatermarkJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(watermarkJob);
    return this.http
      .patch<RestWatermarkJob>(`${this.resourceUrl}/${encodeURIComponent(this.getWatermarkJobIdentifier(watermarkJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestWatermarkJob>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
