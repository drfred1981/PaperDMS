...
export class RedactionJobService {
  ...
  update(redactionJob: IRedactionJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(redactionJob);
    return this.http
      .put<RestRedactionJob>(`${this.resourceUrl}/${encodeURIComponent(this.getRedactionJobIdentifier(redactionJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(redactionJob: PartialUpdateRedactionJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(redactionJob);
    return this.http
      .patch<RestRedactionJob>(`${this.resourceUrl}/${encodeURIComponent(this.getRedactionJobIdentifier(redactionJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestRedactionJob>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
