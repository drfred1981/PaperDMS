...
@Component({
  ...
  templateUrl: './export-job-update.html',
  imports: [SharedModule, ReactiveFormsModule],
})
export class ExportJobUpdate implements OnInit {
  isSaving = false;
  exportJob: IExportJob | null = null;
  exportFormatValues = Object.keys(ExportFormat);
  exportStatusValues = Object.keys(ExportStatus);

  exportPatternsSharedCollection = signal<IExportPattern[]>([]);
  ...
  protected updateForm(exportJob: IExportJob): void {
    ...
    this.exportPatternsSharedCollection.set(
      this.exportPatternService.addExportPatternToCollectionIfMissing<IExportPattern>(
  ...
  protected loadRelationshipsOptions(): void {
    this.exportPatternService
      ...
      .subscribe((exportPatterns: IExportPattern[]) => this.exportPatternsSharedCollection.set(exportPatterns));
  }
}
