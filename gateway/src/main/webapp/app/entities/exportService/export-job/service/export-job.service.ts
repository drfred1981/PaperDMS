...
export class ExportJobService {
  ...
  update(exportJob: IExportJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(exportJob);
    return this.http
      .put<RestExportJob>(`${this.resourceUrl}/${encodeURIComponent(this.getExportJobIdentifier(exportJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(exportJob: PartialUpdateExportJob): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(exportJob);
    return this.http
      .patch<RestExportJob>(`${this.resourceUrl}/${encodeURIComponent(this.getExportJobIdentifier(exportJob))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestExportJob>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
