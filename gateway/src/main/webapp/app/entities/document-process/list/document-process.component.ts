...
import { DocumentProcessDeleteDialogComponent } from '../delete/document-process-delete-dialog.component';
...
export class DocumentProcessComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && DocumentProcessComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(documentProcess: IDocumentProcess): void {
    const modalRef = this.modalService.open(DocumentProcessDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DocumentProcessComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
