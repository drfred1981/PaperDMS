...
import { SearchIndexDeleteDialogComponent } from '../delete/search-index-delete-dialog.component';
...
export class SearchIndexComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && SearchIndexComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(searchIndex: ISearchIndex): void {
    const modalRef = this.modalService.open(SearchIndexDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && SearchIndexComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ISearchIndex[] | null): ISearchIndex[] {
    ...
    if (this.links().prev) {
      const searchIndicesNew = this.searchIndices();
      if (data) {
        for (const d of data) {
          if (searchIndicesNew.some(op => op.id === d.id)) {
            searchIndicesNew.push(d);
          }
        }
      }
      return searchIndicesNew;
    }
    return data ?? [];
  }
