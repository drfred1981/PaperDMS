...
import { ManualDeleteDialogComponent } from '../delete/manual-delete-dialog.component';
...
export class ManualComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && ManualComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(manual: IManual): void {
    const modalRef = this.modalService.open(ManualDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ManualComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
