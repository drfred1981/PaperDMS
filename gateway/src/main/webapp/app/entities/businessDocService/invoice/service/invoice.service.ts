...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'issueDate' | 'dueDate' | 'paymentDate' | 'createdDate'> & {
...
export class InvoiceService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/invoices', 'businessdocservice');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/invoices/_search', 'businessdocservice');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestInvoice[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),

      catchError(() => scheduled([new HttpResponse<IInvoice[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...
      paymentDate: invoice.paymentDate?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...
      paymentDate: restInvoice.paymentDate ? dayjs(restInvoice.paymentDate) : undefined,
