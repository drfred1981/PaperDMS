...
import { ContractDeleteDialog } from '../delete/contract-delete-dialog';
...
export class Contract implements OnInit {
  ...
  protected readonly contractService = inject(ContractService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);

  trackId = (item: IContract): number => this.contractService.getContractIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && Contract.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  delete(contract: IContract): void {
    const modalRef = this.modalService.open(ContractDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && Contract.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.contracts.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.contractService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
