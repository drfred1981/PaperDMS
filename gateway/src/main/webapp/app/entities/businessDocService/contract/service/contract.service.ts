...
type RestOf<T extends IContract | NewContract> = Omit<T, 'startDate' | 'endDate' | 'createdDate'> & {
...
export class ContractService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/contracts', 'businessdocservice');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/contracts/_search', 'businessdocservice');
  ...
  protected convertDateFromClient<T extends IContract | NewContract | PartialUpdateContract>(contract: T): RestOf<T> {
    return {
      ...
      createdDate: contract.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContract: RestContract): IContract {
    return {
      ...
      createdDate: restContract.createdDate ? dayjs(restContract.createdDate) : undefined,
    };
  }
