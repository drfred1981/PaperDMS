...
export class OcrResultService {
  ...
  update(ocrResult: IOcrResult): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(ocrResult);
    return this.http
      .put<RestOcrResult>(`${this.resourceUrl}/${encodeURIComponent(this.getOcrResultIdentifier(ocrResult))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(ocrResult: PartialUpdateOcrResult): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(ocrResult);
    return this.http
      .patch<RestOcrResult>(`${this.resourceUrl}/${encodeURIComponent(this.getOcrResultIdentifier(ocrResult))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestOcrResult>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
