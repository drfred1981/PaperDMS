...
export class ScanBatchService {
  ...
  update(scanBatch: IScanBatch): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(scanBatch);
    return this.http
      .put<RestScanBatch>(`${this.resourceUrl}/${this.getScanBatchIdentifier(scanBatch)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(scanBatch: PartialUpdateScanBatch): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(scanBatch);
    return this.http
      .patch<RestScanBatch>(`${this.resourceUrl}/${this.getScanBatchIdentifier(scanBatch)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestScanBatch>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
