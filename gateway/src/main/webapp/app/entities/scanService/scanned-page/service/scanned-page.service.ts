...
export class ScannedPageService {
  ...
  update(scannedPage: IScannedPage): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(scannedPage);
    return this.http
      .put<RestScannedPage>(`${this.resourceUrl}/${encodeURIComponent(this.getScannedPageIdentifier(scannedPage))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(scannedPage: PartialUpdateScannedPage): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(scannedPage);
    return this.http
      .patch<RestScannedPage>(`${this.resourceUrl}/${encodeURIComponent(this.getScannedPageIdentifier(scannedPage))}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestScannedPage>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
