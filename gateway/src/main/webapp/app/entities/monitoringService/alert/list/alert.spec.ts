...
import { sampleWithRequiredData } from '../alert.test-samples';
...
describe('Alert Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.alerts()[0]).toEqual(expect.objectContaining({ id: 22054 }));
  });

  describe('trackId', () => {
    it('should forward to alertService', () => {
      const entity = { id: 22054 };
      jest.spyOn(service, 'getAlertIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAlertIdentifier).toHaveBeenCalledWith(entity);
