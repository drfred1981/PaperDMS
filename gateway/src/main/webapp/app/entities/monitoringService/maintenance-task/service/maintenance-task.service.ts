...
export class MaintenanceTaskService {
  ...
  update(maintenanceTask: IMaintenanceTask): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(maintenanceTask);
    return this.http
      .put<RestMaintenanceTask>(`${this.resourceUrl}/${this.getMaintenanceTaskIdentifier(maintenanceTask)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(maintenanceTask: PartialUpdateMaintenanceTask): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(maintenanceTask);
    return this.http
      .patch<RestMaintenanceTask>(`${this.resourceUrl}/${this.getMaintenanceTaskIdentifier(maintenanceTask)}`, copy, {
        observe: 'response',
      })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestMaintenanceTask>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
