import { Component, inject } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';

import SharedModule from 'app/shared/shared.module';
import { ITEM_DELETED_EVENT } from 'app/config/navigation.constants';
import { IMaintenanceTask } from '../maintenance-task.model';
import { MaintenanceTaskService } from '../service/maintenance-task.service';

@Component({
  templateUrl: './maintenance-task-delete-dialog.component.html',
  imports: [SharedModule, FormsModule],
})
export class MaintenanceTaskDeleteDialogComponent {
  maintenanceTask?: IMaintenanceTask;

  protected maintenanceTaskService = inject(MaintenanceTaskService);
  protected activeModal = inject(NgbActiveModal);

  cancel(): void {
    this.activeModal.dismiss();
  }

  confirmDelete(id: number): void {
    this.maintenanceTaskService.delete(id).subscribe(() => {
      this.activeModal.close(ITEM_DELETED_EVENT);
    });
  }
}
