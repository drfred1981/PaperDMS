...
export class ImportRuleService {
  ...
  update(importRule: IImportRule): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(importRule);
    return this.http
      .put<RestImportRule>(`${this.resourceUrl}/${this.getImportRuleIdentifier(importRule)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(importRule: PartialUpdateImportRule): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(importRule);
    return this.http
      .patch<RestImportRule>(`${this.resourceUrl}/${this.getImportRuleIdentifier(importRule)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestImportRule>(`${this.resourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
