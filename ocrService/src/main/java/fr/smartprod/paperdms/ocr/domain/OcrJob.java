...
@org.springframework.data.elasticsearch.annotations.Document(indexName = "ocrjob")
...
public class OcrJob implements Serializable {
    ...
    private Set<OcrResult> results = new HashSet<>();

    @OneToMany(fetch = FetchType.LAZY, mappedBy = "job")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @org.springframework.data.annotation.Transient
    @JsonIgnoreProperties(value = { "job" }, allowSetters = true)
    private Set<OrcExtractedText> extracteds = new HashSet<>();
    ...
    public Set<OcrResult> getResults() {
        return this.results;
    }

    public void setResults(Set<OcrResult> ocrResults) {
        if (this.results != null) {
            this.results.forEach(i -> i.setJob(null));
        }
        if (ocrResults != null) {
            ocrResults.forEach(i -> i.setJob(this));
        }
        this.results = ocrResults;
    }

    public OcrJob results(Set<OcrResult> ocrResults) {
        this.setResults(ocrResults);
        return this;
    }

    public OcrJob addResults(OcrResult ocrResult) {
        this.results.add(ocrResult);
        ocrResult.setJob(this);
        return this;
    }

    public OcrJob removeResults(OcrResult ocrResult) {
        this.results.remove(ocrResult);
        ocrResult.setJob(null);
        return this;
    }

    public Set<OrcExtractedText> getExtracteds() {
        return this.extracteds;
    }

    public void setExtracteds(Set<OrcExtractedText> orcExtractedTexts) {
        if (this.extracteds != null) {
            this.extracteds.forEach(i -> i.setJob(null));
        }
        if (orcExtractedTexts != null) {
            orcExtractedTexts.forEach(i -> i.setJob(this));
        }
        this.extracteds = orcExtractedTexts;
    }

    public OcrJob extracteds(Set<OrcExtractedText> orcExtractedTexts) {
        this.setExtracteds(orcExtractedTexts);
        return this;
    }

    public OcrJob addExtracted(OrcExtractedText orcExtractedText) {
        this.extracteds.add(orcExtractedText);
        orcExtractedText.setJob(this);
        return this;
    }

    public OcrJob removeExtracted(OrcExtractedText orcExtractedText) {
        this.extracteds.remove(orcExtractedText);
        orcExtractedText.setJob(null);
        return this;
    }
