...
class DocumentRelationResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/document-relations/_search";
    ...
    @Autowired
    private DocumentRelationSearchRepository documentRelationSearchRepository;
    ...
    void cleanup() {
        if (insertedDocumentRelation != null) {
            ...
            documentRelationSearchRepository.delete(insertedDocumentRelation);
    ...
    void createDocumentRelation() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(documentRelationSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(documentRelationSearchRepository.findAll());
    ...
    void putExistingDocumentRelation() throws Exception {
        ...
        documentRelationSearchRepository.save(documentRelation);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(documentRelationSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(documentRelationSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<DocumentRelation> documentRelationSearchList = Streamable.of(documentRelationSearchRepository.findAll()).toList();
                DocumentRelation testDocumentRelationSearch = documentRelationSearchList.get(searchDatabaseSizeAfter - 1);

                assertDocumentRelationAllPropertiesEquals(testDocumentRelationSearch, updatedDocumentRelation);
            });
    }
    ...
    @Test
    @Transactional
    void searchDocumentRelation() throws Exception {
        // Initialize the database
        insertedDocumentRelation = documentRelationRepository.saveAndFlush(documentRelation);
        documentRelationSearchRepository.save(documentRelation);

        // Search the documentRelation
        restDocumentRelationMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + documentRelation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(documentRelation.getId().intValue())))
            .andExpect(jsonPath("$.[*].sourceDocumentId").value(hasItem(DEFAULT_SOURCE_DOCUMENT_ID.intValue())))
