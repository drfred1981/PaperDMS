...
import fr.smartprod.paperdms.document.logic.upload.service.DocumentEventPublisher;
import fr.smartprod.paperdms.document.logic.upload.service.DocumentServiceStatusUploadService;
...
@Service
@Transactional
public class DocumentServiceStatusUploadServiceImpl implements DocumentServiceStatusUploadService {
	...
	public DocumentServiceStatusUploadServiceImpl(DocumentServiceStatusUploadRepository documentServiceStatusRepository,
			DocumentServiceStatusMapper documentServiceStatusMapper, DocumentEventPublisher documentEventPublisher) {
	...
	public DocumentServiceStatusDTO updateServiceStatus(Long documentId, ServiceType serviceType, ServiceStatus status,
		...
		log.info("Updating service status: documentId={}, serviceType={}, status={}", documentId, serviceType, status);

		DocumentServiceStatus serviceStatus = documentServiceStatusRepository
				.findByDocumentIdAndServiceTypeWithLock(documentId, serviceType).orElseGet(() -> {
					log.debug("Creating new service status record: documentId={}, serviceType={}", documentId,
							serviceType);
		...
		if (shouldUpdateStatus(serviceStatus, status)) {
			...
			log.info(
					"Service status updated successfully: documentId={}, serviceType={}, previousStatus={}, newStatus={}",
	...
	public DocumentServiceStatusDTO updateServiceStatusWithError(Long documentId, ServiceType serviceType,
		...
		log.error("Updating service status with error: documentId={}, serviceType={}, error={}", documentId,
		...
		if (serviceStatus.getProcessingStartDate() != null && serviceStatus.getProcessingEndDate() == null) {
			...
			serviceStatus.setProcessingDuration(serviceStatus.getProcessingEndDate().toEpochMilli()
	...
	public DocumentServiceStatusDTO markProcessingStarted(Long documentId, ServiceType serviceType, String jobId,
			String userId) {
		log.info("Marking processing started: documentId={}, serviceType={}, jobId={}", documentId, serviceType, jobId);
	...
	public DocumentServiceStatusDTO markProcessingCompleted(Long documentId, ServiceType serviceType,
			String statusDetails, String userId) {
		log.info("Marking processing completed: documentId={}, serviceType={}", documentId, serviceType);

		DocumentServiceStatus serviceStatus = documentServiceStatusRepository
				.findByDocumentIdAndServiceTypeWithLock(documentId, serviceType)
				.orElseThrow(() -> new IllegalStateException(String
						.format("Service status not found: documentId=%d, serviceType=%s", documentId, serviceType)));
	...
	public Optional<DocumentServiceStatusDTO> getServiceStatus(Long documentId, ServiceType serviceType) {
		...
		return documentServiceStatusRepository.findByDocumentIdAndServiceType(documentId, serviceType)
				.map(documentServiceStatusMapper::toDto);
	}

	/**
	 * Determine if status should be updated based on current state. Implements
	...
	private boolean shouldUpdateStatus(DocumentServiceStatus serviceStatus, ServiceStatus newStatus) {
		...
		if (currentStatus == ServiceStatus.COMPLETED && newStatus != ServiceStatus.RETRYING) {
			log.warn("Cannot change status from COMPLETED to {} (documentId={}, serviceType={})", newStatus,
					serviceStatus.getDocumentId(), serviceStatus.getServiceType());
			return false;
		}
	 ...
	 * @param userId        The user ID
	 */
	private void updateServiceStatusFields(DocumentServiceStatus serviceStatus, ServiceStatus status,
			String statusDetails, String userId) {
	...
	private void publishServiceStatusEvent(DocumentServiceStatus serviceStatus) {
		DocumentServiceStatusEvent event = new DocumentServiceStatusEvent(serviceStatus.getDocumentId(),
				serviceStatus.getServiceType().name(), serviceStatus.getStatus().name(), "documentService");
