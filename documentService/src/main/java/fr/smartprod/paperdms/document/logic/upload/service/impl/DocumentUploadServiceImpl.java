...
import fr.smartprod.paperdms.common.enumeration.DocumentEventType;
...
public class DocumentUploadServiceImpl implements DocumentUploadService {
	...
	private final DocumentServiceStatusUploadService documentServiceStatusService;

	public DocumentUploadServiceImpl(DocumentRepository documentRepository, DocumentMapper documentMapper,
			S3StorageService s3StorageService, DocumentEventPublisher documentEventPublisher,
			DocumentServiceStatusUploadService documentServiceStatusService) {
	...
	public DocumentDTO uploadDocument(MultipartFile file, Long folderId, Long documentTypeId, String userId)
		...
		if (existingDocument.isPresent()) {
			log.info("Document with same SHA-256 already exists (deduplication): documentId={}, sha256={}",
					existingDocument.orElseThrow().getId(), sha256);
			return documentMapper.toDto(existingDocument.orElseThrow());
		}
		...
		try {
			S3StorageService.S3UploadResult uploadResult = s3StorageService.uploadFile(fileData, sha256,
			...
			log.error("Document upload failed: fileName={}, sha256={}, error={}", file.getOriginalFilename(), sha256,
					e.getMessage(), e);

			if (document.getId() != null) {
				updateServiceStatus(document.getId(), ServiceStatus.FAILED, userId);
				documentEventPublisher.publishDocumentEvent(DocumentEventType.DOCUMENT_UPLOAD_FAILED, document.getId(),
						sha256, userId);
			}
	 ...
	 */
	private Document createDocument(MultipartFile file, String sha256, Long folderId, Long documentTypeId,
			String userId) {
	...
	private Integer extractPageCount(byte[] fileData, String contentType) {
		...
		try (PDDocument pdfDocument = org.apache.pdfbox.Loader.loadPDF(fileData)) {
	...
	private void publishUploadStartedEvent(Document document, String userId) {
		...
		DocumentUploadEvent event = new DocumentUploadEvent(DocumentEventType.DOCUMENT_UPLOAD_STARTED, document.getId(),
				"documentService");
	...
	private void publishUploadCompletedEvent(Document document, String userId) {
		...
		DocumentUploadEvent event = new DocumentUploadEvent(DocumentEventType.DOCUMENT_UPLOADED, document.getId(),
				"documentService");
		...
		documentEventPublisher.publishDocumentEvent(DocumentEventType.DOCUMENT_READY_FOR_OCR, document.getId(),
				document.getSha256(), userId);
	}
	...
	private void updateServiceStatus(Long documentId, ServiceStatus status, String userId) {
		...
		documentServiceStatusService.updateServiceStatus(documentId, ServiceType.OCR_SERVICE,
				status == ServiceStatus.COMPLETED ? ServiceStatus.PENDING : status,
				status == ServiceStatus.COMPLETED ? "Ready for OCR processing" : "Upload failed", userId);
	}
}
