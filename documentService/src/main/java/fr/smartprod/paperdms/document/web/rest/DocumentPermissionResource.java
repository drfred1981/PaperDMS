...
public class DocumentPermissionResource {
    ...
    /**
     * {@code SEARCH  /document-permissions/_search?query=:query} : search for the documentPermission corresponding
     * to the query.
     *
     * @param query the query of the documentPermission search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentPermissionDTO>> searchDocumentPermissions(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of DocumentPermissions for query {}", query);
        try {
            Page<DocumentPermissionDTO> page = documentPermissionService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
