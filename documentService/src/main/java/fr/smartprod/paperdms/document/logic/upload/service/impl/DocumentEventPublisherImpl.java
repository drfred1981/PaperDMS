...
public class DocumentEventPublisherImpl {
	...
	private static final Logger log = LoggerFactory.getLogger(DocumentEventPublisherImpl.class);
	...
	public DocumentEventPublisherImpl(KafkaTemplate<String, DocumentEvent> kafkaTemplate) {
	...
	public void publishUploadEvent(DocumentUploadEvent event) {
		...
		log.info("Publishing document upload event: eventId={}, documentId={}, eventType={}", event.getEventId(),
				event.getDocumentId(), event.getEventType());

		try {
			kafkaTemplate.send(documentEventsTopic, key, event).whenComplete((result, ex) -> {
				if (ex != null) {
					log.error("Failed to publish document upload event: eventId={}, error={}", event.getEventId(),
							ex.getMessage(), ex);
				} else {
			...
			log.error("Exception while publishing document upload event: eventId={}, error={}", event.getEventId(),
					e.getMessage(), e);
	...
	public void publishServiceStatusEvent(DocumentServiceStatusEvent event) {
		...
		try {
			kafkaTemplate.send(serviceStatusTopic, key, event).whenComplete((result, ex) -> {
				if (ex != null) {
					log.error("Failed to publish service status event: eventId={}, error={}", event.getEventId(),
							ex.getMessage(), ex);
				} else {
			...
			log.error("Exception while publishing service status event: eventId={}, error={}", event.getEventId(),
					e.getMessage(), e);
	...
	public void publishDocumentEvent(DocumentEventType eventType, Long documentId, String sha256, String userId) {
		...
		log.info("Publishing document event: eventId={}, documentId={}, eventType={}", event.getEventId(), documentId,
				eventType);

		try {
			kafkaTemplate.send(documentEventsTopic, key, event).whenComplete((result, ex) -> {
				if (ex != null) {
					log.error("Failed to publish document event: eventId={}, error={}", event.getEventId(),
							ex.getMessage(), ex);
				} else {
			...
			log.error("Exception while publishing document event: eventId={}, error={}", event.getEventId(),
					e.getMessage(), e);
	...
	/**
	 * Generate a Kafka message key for event partitioning. Uses documentId to
	 * ensure all events for a document go to the same partition.
	 *
	 * @param documentId The document ID
