...
class WorkflowResourceIT {
    ...
    void cleanup() {
        if (insertedWorkflow != null) {
            ...
            workflowSearchRepository.delete(insertedWorkflow);
    ...
    void createWorkflow() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(workflowSearchRepository.findAll());
        // Create the Workflow
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(workflowSearchRepository.findAll());
    ...
    void putExistingWorkflow() throws Exception {
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(workflowSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Workflow> workflowSearchList = Streamable.of(workflowSearchRepository.findAll()).toList();
                Workflow testWorkflowSearch = workflowSearchList.get(searchDatabaseSizeAfter - 1);

                assertWorkflowAllPropertiesEquals(testWorkflowSearch, updatedWorkflow);
            });
    }
    ...
    @Test
    @Transactional
    void searchWorkflow() throws Exception {
        // Initialize the database
        insertedWorkflow = workflowRepository.saveAndFlush(workflow);
        workflowSearchRepository.save(workflow);
        ...
        // Search the workflow
        restWorkflowMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + workflow.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(workflow.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
