...
class WorkflowTaskResourceIT {
    ...
    public static WorkflowTask createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, WorkflowInstance.class).isEmpty()) {
            workflowInstance = WorkflowInstanceResourceIT.createEntity(em);
            em.persist(workflowInstance);
            em.flush();
        } else {
            workflowInstance = TestUtil.findAll(em, WorkflowInstance.class).get(0);
        }
        workflowTask.setInstance(workflowInstance);
        // Add required entity
        WorkflowStep workflowStep;
        if (TestUtil.findAll(em, WorkflowStep.class).isEmpty()) {
            workflowStep = WorkflowStepResourceIT.createEntity(em);
            em.persist(workflowStep);
            em.flush();
        } else {
            workflowStep = TestUtil.findAll(em, WorkflowStep.class).get(0);
        }
        workflowTask.setStep(workflowStep);
        return workflowTask;
    }
    ...
    public static WorkflowTask createUpdatedEntity(EntityManager em) {
        WorkflowTask updatedWorkflowTask = new WorkflowTask()
        ...
        WorkflowInstance workflowInstance;
        if (TestUtil.findAll(em, WorkflowInstance.class).isEmpty()) {
            workflowInstance = WorkflowInstanceResourceIT.createUpdatedEntity(em);
            em.persist(workflowInstance);
            em.flush();
        } else {
            workflowInstance = TestUtil.findAll(em, WorkflowInstance.class).get(0);
        }
        updatedWorkflowTask.setInstance(workflowInstance);
        // Add required entity
        WorkflowStep workflowStep;
        if (TestUtil.findAll(em, WorkflowStep.class).isEmpty()) {
            workflowStep = WorkflowStepResourceIT.createUpdatedEntity(em);
            em.persist(workflowStep);
            em.flush();
        } else {
            workflowStep = TestUtil.findAll(em, WorkflowStep.class).get(0);
        }
        updatedWorkflowTask.setStep(workflowStep);
    ...
    void getAllWorkflowTasksByInstanceIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, WorkflowInstance.class).isEmpty()) {
            ...
            instance = WorkflowInstanceResourceIT.createEntity(em);
        } else {
    ...
    void getAllWorkflowTasksByStepIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, WorkflowStep.class).isEmpty()) {
            workflowTaskRepository.saveAndFlush(workflowTask);
            step = WorkflowStepResourceIT.createEntity(em);
        } else {
