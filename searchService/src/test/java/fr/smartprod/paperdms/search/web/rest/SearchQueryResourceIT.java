...
class SearchQueryResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/search-queries/_search";
    ...
    @Autowired
    private SearchQuerySearchRepository searchQuerySearchRepository;
    ...
    void cleanup() {
        if (insertedSearchQuery != null) {
            ...
            searchQuerySearchRepository.delete(insertedSearchQuery);
    ...
    void createSearchQuery() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(searchQuerySearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(searchQuerySearchRepository.findAll());
    ...
    void putExistingSearchQuery() throws Exception {
        ...
        searchQuerySearchRepository.save(searchQuery);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(searchQuerySearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(searchQuerySearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<SearchQuery> searchQuerySearchList = Streamable.of(searchQuerySearchRepository.findAll()).toList();
                SearchQuery testSearchQuerySearch = searchQuerySearchList.get(searchDatabaseSizeAfter - 1);

                assertSearchQueryAllPropertiesEquals(testSearchQuerySearch, updatedSearchQuery);
            });
    }
    ...
    @Test
    @Transactional
    void searchSearchQuery() throws Exception {
        // Initialize the database
        insertedSearchQuery = searchQueryRepository.saveAndFlush(searchQuery);
        searchQuerySearchRepository.save(searchQuery);
        ...
        restSearchQueryMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + searchQuery.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(searchQuery.getId().intValue())))
            .andExpect(jsonPath("$.[*].query").value(hasItem(DEFAULT_QUERY)))
            .andExpect(jsonPath("$.[*].filters").value(hasItem(DEFAULT_FILTERS.toString())))
            .andExpect(jsonPath("$.[*].resultCount").value(hasItem(DEFAULT_RESULT_COUNT)))
            .andExpect(jsonPath("$.[*].executionTime").value(hasItem(DEFAULT_EXECUTION_TIME.intValue())))
            .andExpect(jsonPath("$.[*].userId").value(hasItem(DEFAULT_USER_ID)))
            .andExpect(jsonPath("$.[*].searchDate").value(hasItem(DEFAULT_SEARCH_DATE.toString())))
