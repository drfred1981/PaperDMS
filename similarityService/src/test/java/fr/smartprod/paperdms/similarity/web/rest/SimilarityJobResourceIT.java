...
class SimilarityJobResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/similarity-jobs/_search";
    ...
    @Autowired
    private SimilarityJobSearchRepository similarityJobSearchRepository;
    ...
    void cleanup() {
        if (insertedSimilarityJob != null) {
            ...
            similarityJobSearchRepository.delete(insertedSimilarityJob);
    ...
    void createSimilarityJob() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(similarityJobSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(similarityJobSearchRepository.findAll());
    ...
    void putExistingSimilarityJob() throws Exception {
        ...
        similarityJobSearchRepository.save(similarityJob);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(similarityJobSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(similarityJobSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<SimilarityJob> similarityJobSearchList = Streamable.of(similarityJobSearchRepository.findAll()).toList();
                SimilarityJob testSimilarityJobSearch = similarityJobSearchList.get(searchDatabaseSizeAfter - 1);

                assertSimilarityJobAllPropertiesEquals(testSimilarityJobSearch, updatedSimilarityJob);
            });
    }
    ...
    void deleteSimilarityJob() throws Exception {
        // Initialize the database
        insertedSimilarityJob = similarityJobRepository.saveAndFlush(similarityJob);
        similarityJobRepository.save(similarityJob);
        similarityJobSearchRepository.save(similarityJob);
    ...
    @Test
    @Transactional
    void searchSimilarityJob() throws Exception {
        // Initialize the database
        insertedSimilarityJob = similarityJobRepository.saveAndFlush(similarityJob);
        similarityJobSearchRepository.save(similarityJob);

        // Search the similarityJob
        restSimilarityJobMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + similarityJob.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(similarityJob.getId().intValue())))
            .andExpect(jsonPath("$.[*].documentSha256").value(hasItem(DEFAULT_DOCUMENT_SHA_256)))
