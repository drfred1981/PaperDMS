...
/**
 * REST controller for managing {@link fr.smartprod.paperdms.similarity.domain.DocumentFingerprint}.
 */
@RestController
...
public class DocumentFingerprintResource {
    ...
    /**
     * {@code SEARCH  /document-fingerprints/_search?query=:query} : search for the documentFingerprint corresponding
     * to the query.
     *
     * @param query the query of the documentFingerprint search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentFingerprintDTO>> searchDocumentFingerprints(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of DocumentFingerprints for query {}", query);
        try {
            Page<DocumentFingerprintDTO> page = documentFingerprintService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
