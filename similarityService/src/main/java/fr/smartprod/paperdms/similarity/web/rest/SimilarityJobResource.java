...
import fr.smartprod.paperdms.similarity.service.SimilarityJobQueryService;
...
/**
 * REST controller for managing {@link fr.smartprod.paperdms.similarity.domain.SimilarityJob}.
 */
@RestController
...
public class SimilarityJobResource {
    ...
    /**
     * {@code SEARCH  /similarity-jobs/_search?query=:query} : search for the similarityJob corresponding
     * to the query.
     *
     * @param query the query of the similarityJob search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<SimilarityJobDTO>> searchSimilarityJobs(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of SimilarityJobs for query {}", query);
        try {
            Page<SimilarityJobDTO> page = similarityJobService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
